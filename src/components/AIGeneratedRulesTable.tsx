...
import { ChevronDown, ChevronRight, CheckCircle2, XCircle, AlertTriangle, Code } from 'lucide-react';
import { 
  Table, 
  TableBody, 
  TableCell, 
...
const resultConfig = {
  ...
  pending: { icon: AlertTriangle, color: 'text-muted-foreground', bg: 'bg-muted/10', label: 'Pending' },
};
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border/30 hover:bg-transparent">
            ...
            <TableHead className="w-[160px]">Rule Generated</TableHead>
...
function TableSection({ tableName, rules, defaultOpen = false }: TableSectionProps) {
  ...
  const pendingCount = rules.filter(r => r.result.status === 'pending').length;
...
interface AIGeneratedRulesTableProps {
  rules: AIGeneratedRule[];
}

export function AIGeneratedRulesTable({ rules }: AIGeneratedRulesTableProps) {
  // Group rules by tableName
  const groupedRules = rules.reduce((acc, rule) => {
    if (!acc[rule.tableName]) acc[rule.tableName] = [];
    acc[rule.tableName].push(rule);
    return acc;
  }, {} as Record<string, AIGeneratedRule[]>);
